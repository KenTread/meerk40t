...
//
// .m4d is the only format OmniFractal writes. Everything else is import-only.
//
...
// from a newer build is refused with OMF_ERR_FORMAT_VERSION rather than
...
struct M4dHeader {
    char          magic[4];          // "MB4D"
    std::uint32_t format_version;    // OMF_M4D_FORMAT_VERSION
    std::uint32_t writer_abi;        // OMF_ABI_VERSION of the writer
    std::uint32_t flags;
...
struct Document {
    ...
    std::optional<omf_render_settings_t> legacy;
...
// Both throw omf::Error on failure. Every read is bounds-checked
