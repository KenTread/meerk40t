...
void f_bulb3d(IterationState& s, const omf_formula_slot_t& cfg) {
...
// -- 4D / quaternion bulb --------------------------------------------
void f_bulb4d(IterationState& s, const omf_formula_slot_t& cfg) {
    const double power = cfg.params[0] != 0.0f ? cfg.params[0] : 8.0;
...
void f_mandelbox(IterationState& s, const omf_formula_slot_t& cfg) {
...
void f_fold(IterationState& s, const omf_formula_slot_t& cfg) {
...
void f_transform(IterationState& s, const omf_formula_slot_t& cfg) {
...
void f_ifs(IterationState& s, const omf_formula_slot_t& cfg) {
    const double scale = cfg.params[0] != 0.0f ? cfg.params[0] : 2.0;
...
FormulaRegistry::FormulaRegistry() {
    add({"Bulb3D",      OMF_FORMULA_BULB3D,    &f_bulb3d,    kGlslBulb3d,    {}, true});
    add({"Bulb4D",      OMF_FORMULA_BULB4D,    &f_bulb4d,    {},             {}, true});
    add({"Mandelbox",   OMF_FORMULA_BOX,       &f_mandelbox, kGlslMandelbox, {}, true});
    add({"TgladFold",   OMF_FORMULA_BOX,       &f_mandelbox, kGlslMandelbox, {}, true});
    add({"KaleidoIFS",  OMF_FORMULA_FOLD,      &f_fold,      {},             {}, true});
    add({"MengerFold",  OMF_FORMULA_FOLD,      &f_fold,      {},             {}, true});
    add({"Transform",   OMF_FORMULA_TRANSFORM, &f_transform, {},             {}, true});
    add({"IFS",         OMF_FORMULA_IFS,       &f_ifs,       {},             {}, true});
}
...
FormulaDef FormulaRegistry::parse_formula_source(std::string_view name,
    ...
    def.kind = kind == OMF_FORMULA_NONE ? OMF_FORMULA_CUSTOM : kind;
    ...
    require(open != std::string_view::npos && close != std::string_view::npos && close > open,
            OMF_ERR_PARSE, "formula source has no { ... } body");
    ...
    switch (def.kind) {
        case OMF_FORMULA_BOX:       def.fn = &f_mandelbox; break;
        case OMF_FORMULA_FOLD:      def.fn = &f_fold;      break;
        case OMF_FORMULA_TRANSFORM: def.fn = &f_transform; break;
        case OMF_FORMULA_IFS:       def.fn = &f_ifs;       break;
        case OMF_FORMULA_BULB4D:    def.fn = &f_bulb4d;    break;
...
FormulaPipeline FormulaPipeline::build(const FormulaRegistry& reg,
    ...
    pipe.master = static_cast<omf_hybrid_mode_t>(settings.hybrid_master_mode);

    for (int i = 0; i < OMF_FORMULA_SLOTS; ++i) {
        const omf_formula_slot_t& cfg = settings.formulas[i];
        if (!cfg.enabled) continue;

        std::string name(cfg.name, ::strnlen(cfg.name, OMF_MAX_NAME));
        ...
        if (def == nullptr) {
            ...
            switch (cfg.kind) {
                case OMF_FORMULA_BOX:       def = reg.find("Mandelbox"); break;
                case OMF_FORMULA_FOLD:      def = reg.find("KaleidoIFS"); break;
                case OMF_FORMULA_TRANSFORM: def = reg.find("Transform"); break;
                case OMF_FORMULA_IFS:       def = reg.find("IFS"); break;
                case OMF_FORMULA_BULB4D:    def = reg.find("Bulb4D"); break;
...
double orbit_trap_distance(const omf_coloring_settings_t& c, const Vec3& z) {
...
inline const FormulaPipeline::Slot* pick_slot(const FormulaPipeline& pipe, int i) {
    ...
    switch (pipe.master) {
        case OMF_HYBRID_ALTERNATE:
            return &pipe.slots[static_cast<std::size_t>(i) % n];

        case OMF_HYBRID_DECOMB:
        ...
        case OMF_HYBRID_SEQUENTIAL:
        case OMF_HYBRID_INTERPOLATE:
        default: {
...
double distance_estimate(const FormulaPipeline& pipe,
    ...
    for (int i = 0; i < max_iter; ++i) {
        ...
        const omf_formula_slot_t& cfg = slot->cfg;
        ...
        if (cfg.rotation[0] != 0.0f || cfg.rotation[1] != 0.0f || cfg.rotation[2] != 0.0f) {
            if (cfg.kind != OMF_FORMULA_TRANSFORM) {
                s.z = rotate_euler(s.z, cfg.rotation[0], cfg.rotation[1], cfg.rotation[2]);
            }
        }

        if (pipe.master == OMF_HYBRID_INTERPOLATE && pipe.slots.size() > 1) {
        ...
        if (color.trap_kind != OMF_TRAP_NONE && i >= color.trap_min_iteration) {
...
Rgb sample_gradient(const omf_coloring_settings_t& c, double t) {
    ...
    const double scaled = u * OMF_GRADIENT_ENTRIES;
    const int i0 = static_cast<int>(scaled) % OMF_GRADIENT_ENTRIES;
    const int i1 = (i0 + 1) % OMF_GRADIENT_ENTRIES;
