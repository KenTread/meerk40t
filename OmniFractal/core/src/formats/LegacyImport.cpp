...
struct ContainerHeader {
    omf_file_kind_t kind{OMF_FILE_UNKNOWN};
...
ContainerHeader parse_container(std::span<const std::byte> data, omf_file_kind_t expected) {
    require(data.size() >= 16, OMF_ERR_CORRUPT, "file is too small to be a legacy container");
    ...
    require(header.payload_size > 0, OMF_ERR_CORRUPT, "legacy container has no payload");
    return header;
}
...
std::vector<Record> walk_records(std::span<const std::byte> stream, int& unmapped_count) {
    ...
    while (reader.remaining() >= 10) {
        ...
        require(length <= reader.remaining(), OMF_ERR_CORRUPT,
        ...
        require(records.size() <= Limits::kMaxRecords, OMF_ERR_LIMIT_EXCEEDED,
...
void decode_camera(const Record& record, omf_camera_transform_t& camera) {
...
void decode_camera_angles(const Record& record, omf_camera_transform_t& camera) {
    ByteReader reader(record.payload);
    if (reader.remaining() < 3 * sizeof(double)) return;
    for (double& v : camera.rotation) v = reader.f64();
}

void decode_calc_basic(const Record& record, omf_calculation_settings_t& calc) {
    ByteReader reader(record.payload);
...
void decode_calc_sampling(const Record& record, omf_calculation_settings_t& calc) {
    ByteReader reader(record.payload);
...
void decode_calc_bounds(const Record& record, omf_calculation_settings_t& calc) {
    ByteReader reader(record.payload);
...
void decode_formula_slot(const Record& record, omf_render_settings_t& settings,
                         omf_import_report_t& report) {
    if (record.index >= OMF_FORMULA_SLOTS) {
        // Out-of-range slot index: count it and move on rather than
        // writing past the array.
        ++report.unmapped_field_count;
        return;
    }
    omf_formula_slot_t& slot = settings.formulas[record.index];
    ...
    const std::string legacy_name = reader.fixed_string(
        std::min<std::size_t>(OMF_MAX_NAME, reader.remaining()));
    ...
    slot.repeat_from_slot =
        (slot.repeat_from_slot >= 0 && slot.repeat_from_slot < OMF_FORMULA_SLOTS)
    ...
    if (const auto resolved = resolve_formula_name(legacy_name)) {
        ...
        slot.kind = OMF_FORMULA_IMPORTED;
    } else {
        set_fixed(slot.name, sizeof(slot.name), legacy_name);
        slot.enabled = 0;
        slot.kind = OMF_FORMULA_NONE;
        ++report.unresolved_formula_count;
    }
}

void decode_light_channel(const Record& record, omf_lighting_settings_t& lighting,
                          omf_import_report_t& report) {
    if (record.index >= OMF_LIGHT_CHANNELS) {
        ++report.unmapped_field_count;
        return;
    }
    omf_light_channel_t& channel = lighting.channels[record.index];
    ...
    if (reader.remaining() > 0) {
        const std::string map_name =
            reader.fixed_string(std::min<std::size_t>(OMF_MAX_NAME, reader.remaining()));
        set_fixed(channel.lightmap, sizeof(channel.lightmap), map_name);
    }
}

void decode_gradient(const Record& record, omf_coloring_settings_t& coloring) {
    ByteReader reader(record.payload);
    // Entries beyond 256 are ignored; fewer than 256 leaves the tail at
    // the engine default rather than at black.
    const std::size_t count =
        std::min<std::size_t>(OMF_GRADIENT_ENTRIES, reader.remaining() / 4);
    for (std::size_t i = 0; i < count; ++i) {
        coloring.gradient[i] = 0xFF000000u | (reader.u32() & 0x00FFFFFFu);
    }
}

void decode_orbit_trap(const Record& record, omf_coloring_settings_t& coloring) {
    ByteReader reader(record.payload);
...
void decode_keyframe(const Record& record, std::vector<omf_keyframe_t>& keyframes) {
    require(keyframes.size() < Limits::kMaxKeyframes, OMF_ERR_LIMIT_EXCEEDED,
...
void apply_records(const std::vector<Record>& records, ImportResult& result) {
    omf_render_settings_t& settings = result.settings;
...
void finalise(ImportResult& result, omf_file_kind_t kind, const ContainerHeader& header,
              std::string_view profile_id, std::string_view source_name,
              std::size_t bytes_read) {
    omf_precision_profile_t profile{};
    const omf_result_t rc =
        omf_precision_profile_by_id(std::string(profile_id).c_str(), &profile);
    require(rc == OMF_OK, OMF_ERR_INVALID_ARG,
    ...
    result.provenance.importer_abi_version = OMF_ABI_VERSION;
    ...
    std::sort(result.keyframes.begin(), result.keyframes.end(),
              [](const omf_keyframe_t& a, const omf_keyframe_t& b) {
                  return a.frame < b.frame;
              });
}

// Shared path for the three compressed parameter containers.
ImportResult import_compressed(std::span<const std::byte> data, omf_file_kind_t kind,
    ...
    ImportResult result;
    omf_render_settings_default(&result.settings);
...
M3iFileHeader read_m3i_header(std::span<const std::byte> data) {
    require(data.size() >= sizeof(M3iFileHeader), OMF_ERR_CORRUPT,
            "file is too small to be an .m3i container");

    M3iFileHeader header{};
    std::memcpy(&header, data.data(), sizeof(header));

    require(std::memcmp(header.magic, "M3I", 3) == 0, OMF_ERR_PARSE,
    ...
    require(header.width > 0 && header.height > 0 &&
            ...
            OMF_ERR_CORRUPT, "'.m3i' declares implausible dimensions");
    require(header.layer_count <= Limits::kMaxLayers, OMF_ERR_LIMIT_EXCEEDED,
...
std::string ByteReader::fixed_string(std::size_t count) {
    require(count <= Limits::kMaxStringBytes, OMF_ERR_LIMIT_EXCEEDED,
...
std::string ByteReader::prefixed_string(std::size_t length_bytes) {
    ...
    switch (length_bytes) {
        ...
        default: fail(OMF_ERR_INVALID_ARG, "unsupported string length prefix width");
    }
    require(length <= Limits::kMaxStringBytes, OMF_ERR_LIMIT_EXCEEDED,
            "string field exceeds the import limit");
...
std::vector<std::byte> inflate(std::span<const std::byte> compressed,
    ...
    require(!compressed.empty(), OMF_ERR_CORRUPT, "empty compressed payload");

#if !defined(OMF_WITH_MINIZ)
    (void)expected_size;
    fail(OMF_ERR_UNSUPPORTED,
         "legacy import requires DEFLATE support; rebuild with -DOMF_WITH_MINIZ=ON");
#else
    ...
    require(status == MZ_OK, OMF_ERR_UNKNOWN, "failed to initialise the inflater");
    ...
    for (;;) {
        ...
        if (status == MZ_OK && stream.avail_out == 0) {
            ...
            require(output.size() < hard_cap, OMF_ERR_LIMIT_EXCEEDED,
        ...
        if (status == MZ_OK || status == MZ_BUF_ERROR) {
            ...
            fail(OMF_ERR_CORRUPT, "compressed payload is truncated");
        }

        mz_inflateEnd(&stream);
        fail(OMF_ERR_CORRUPT,
             std::string("DEFLATE stream is invalid (miniz status ") +
             std::to_string(status) + ")");
    }

    mz_inflateEnd(&stream);
    require(produced > 0, OMF_ERR_CORRUPT, "decompressed payload is empty");
...
omf_file_kind_t identify_bytes(std::span<const std::byte> head, std::string_view extension) {
    ...
    if (head.size() >= 4) {
        ...
        if (std::memcmp(bytes, "MB4D", 4) == 0) return OMF_FILE_M4D;
        if (std::memcmp(bytes, "M3I", 3) == 0)  return OMF_FILE_M3I;
        if (std::memcmp(bytes, "M3V", 3) == 0)  return OMF_FILE_M3V;
    }

    const std::string ext = to_lower(extension);
    if (ext == ".m4d") return OMF_FILE_M4D;
    if (ext == ".m3p") return OMF_FILE_M3P;
    if (ext == ".m3i") return OMF_FILE_M3I;
    if (ext == ".m3a") return OMF_FILE_M3A;
    if (ext == ".m3l") return OMF_FILE_M3L;
    if (ext == ".m3c") return OMF_FILE_M3C;
    if (ext == ".m3v") return OMF_FILE_M3V;
    if (ext == ".m3f") return OMF_FILE_M3F;
    if (ext == ".d3f") return OMF_FILE_D3F;
    if (ext == ".dso") return OMF_FILE_DSO;
    ...
    if (head.size() >= 8) {
        ...
        if (to_lower(text).find("mandelbulb3d") != std::string::npos) return OMF_FILE_M3P;
    }
...
std::vector<std::byte> read_file(const std::string& path) {
    ...
    require(file.good(), OMF_ERR_IO, "cannot open '" + path + "'");

    const auto size = static_cast<std::uint64_t>(file.tellg());
    require(size > 0, OMF_ERR_CORRUPT, "'" + path + "' is empty");
    require(size <= Limits::kMaxFileBytes, OMF_ERR_LIMIT_EXCEEDED,
    ...
    require(file.good(), OMF_ERR_IO, "short read on '" + path + "'");
    return data;
}
...
ImportResult import_m3p(std::span<const std::byte> data, std::string_view profile_id) {
    return import_compressed(data, OMF_FILE_M3P, profile_id);
}

ImportResult import_m3a(std::span<const std::byte> data, std::string_view profile_id) {
    ImportResult result = import_compressed(data, OMF_FILE_M3A, profile_id);
    require(!result.keyframes.empty(), OMF_ERR_PARSE,
            "'.m3a' timeline contains no keyframes");
    return result;
}

ImportResult import_m3l(std::span<const std::byte> data, std::string_view profile_id) {
    return import_compressed(data, OMF_FILE_M3L, profile_id);
}
...
std::vector<std::byte> extract_embedded_parameters(std::span<const std::byte> data) {
    ...
    require(header.parameter_size > 0, OMF_ERR_PARSE,
            "'.m3i' carries no embedded parameter block");
    require(header.parameter_size <= Limits::kMaxParameterBytes, OMF_ERR_LIMIT_EXCEEDED,
    ...
    require(header.parameter_offset <= data.size() &&
            ...
            OMF_ERR_CORRUPT, "embedded parameter block lies outside the file");
...
ImportResult import_m3i(std::span<const std::byte> data, std::string_view profile_id) {
    ...
    require(header.directory_offset <= data.size() &&
            ...
            OMF_ERR_CORRUPT, "'.m3i' layer directory lies outside the file");
    ...
    for (const M3iLayerEntry& entry : directory) {
        require(entry.width == header.width && entry.height == header.height,
                OMF_ERR_CORRUPT, "'.m3i' layer disagrees with the container dimensions");
        require(entry.offset <= data.size() &&
                entry.size_stored <= data.size() - entry.offset,
                OMF_ERR_CORRUPT, "'.m3i' layer payload extends past end of file");
    ...
    if (layer_mask != 0) {
        result.image = std::make_unique<FrameBuffer>(static_cast<int>(header.width),
                                                     ...
                                                     layer_mask);
        for (const M3iLayerEntry& entry : directory) {
            Layer* layer = result.image->layer(static_cast<omf_layer_t>(entry.layer));
            ...
            require(entry.size_stored == layer->bytes.size(), OMF_ERR_CORRUPT,
    ...
    result.provenance.source_format = OMF_FILE_M3I;
    result.report.source_format = OMF_FILE_M3I;
    return result;
}
...
ImportResult import_any(std::span<const std::byte> data, omf_file_kind_t kind,
    ...
    ImportResult result;
    switch (kind) {
        case OMF_FILE_M3P: result = import_m3p(data, profile_id); break;
        case OMF_FILE_M3I: result = import_m3i(data, profile_id); break;
        case OMF_FILE_M3A: result = import_m3a(data, profile_id); break;
        case OMF_FILE_M3L: result = import_m3l(data, profile_id); break;

        case OMF_FILE_M3F:
        case OMF_FILE_D3F:
        case OMF_FILE_DSO:
            fail(OMF_ERR_UNTRUSTED,
                 "formula files carry executable code and are never loaded as scenes; "
                 "use omf_inspect_legacy_formula to read their metadata");

        case OMF_FILE_M4D:
            fail(OMF_ERR_INVALID_ARG, "'.m4d' is the native format -- use omf_m4d_read");

        default:
            fail(OMF_ERR_UNSUPPORTED, "unrecognised legacy file kind");
    }
...
FormulaMetadata parse_formula_metadata(std::span<const std::byte> data,
    ...
    require(kind == OMF_FILE_M3F || kind == OMF_FILE_D3F || kind == OMF_FILE_DSO,
            OMF_ERR_INVALID_ARG, "not a formula file kind");
    require(!data.empty(), OMF_ERR_CORRUPT, "formula file is empty");
    ...
    if (class_text == "3da")      meta.formula_class = OMF_CLASS_3DA;
    else if (class_text == "4d")  meta.formula_class = OMF_CLASS_4D;
    else if (class_text == "4da") meta.formula_class = OMF_CLASS_4DA;
    else                          meta.formula_class = OMF_CLASS_3D;

    meta.analytic_de = to_lower(value_of("analyticde")) == "1" ||
                       to_lower(value_of("analyticde")) == "true";

    for (int i = 0; i < OMF_FORMULA_PARAMS; ++i) {
    ...
    require(!meta.name.empty(), OMF_ERR_PARSE,
