...
// then in a OMF_SHADER_DIR override, then in the build tree layout.
std::vector<std::filesystem::path> shader_search_paths(std::string_view name) {
    std::vector<std::filesystem::path> paths;
    if (const char* env = std::getenv("OMF_SHADER_DIR")) {
    ...
    paths.emplace_back(std::filesystem::path(OMF_INSTALL_SHADER_DIR) / name);
    return paths;
}
...
VulkanContext::VulkanContext() {
    available_ = false;
    init_error_ = "built without Vulkan support (OMF_WITH_VULKAN=OFF)";
}
...
GpuBuffer::GpuBuffer(VmaAllocator allocator, VkDeviceSize size, VkBufferUsageFlags usage,
    ...
    const VkResult r = vmaCreateBuffer(allocator_, &info, &alloc, &buffer_, &allocation_,
                                       &alloc_info);
    require(r == VK_SUCCESS, OMF_ERR_OUT_OF_MEMORY, "vmaCreateBuffer failed");
...
void GpuBuffer::upload(const void* data, std::size_t bytes, std::size_t offset) {
    require(mapped_ != nullptr, OMF_ERR_INVALID_ARG, "upload to unmapped buffer");
    require(offset + bytes <= size_, OMF_ERR_INVALID_ARG, "upload overruns buffer");
...
void GpuBuffer::download(void* data, std::size_t bytes, std::size_t offset) const {
    require(mapped_ != nullptr, OMF_ERR_INVALID_ARG, "download from unmapped buffer");
    require(offset + bytes <= size_, OMF_ERR_INVALID_ARG, "download overruns buffer");
...
ComputePipeline::ComputePipeline(VkDevice device, std::span<const std::uint32_t> spirv,
    ...
    require(vkCreateShaderModule(device_, &module_info, nullptr, &module_) == VK_SUCCESS,
            OMF_ERR_SHADER_COMPILE, "vkCreateShaderModule failed");
    ...
    require(vkCreateDescriptorSetLayout(device_, &set_info, nullptr, &set_layout_) == VK_SUCCESS,
            OMF_ERR_UNKNOWN, "vkCreateDescriptorSetLayout failed");
    ...
    require(vkCreatePipelineLayout(device_, &layout_info, nullptr, &layout_) == VK_SUCCESS,
            OMF_ERR_UNKNOWN, "vkCreatePipelineLayout failed");
    ...
    require(vkCreateComputePipelines(device_, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
            ...
            OMF_ERR_SHADER_COMPILE, "vkCreateComputePipelines failed");
}
...
VulkanContext::VulkanContext() {
    ...
    app.pApplicationName = "OmniFractalCore";
    app.applicationVersion = VK_MAKE_VERSION(OMF_VERSION_MAJOR, OMF_VERSION_MINOR,
                                             OMF_VERSION_PATCH);
    app.pEngineName = "OMF";
...
void VulkanContext::submit_blocking(const std::function<void(VkCommandBuffer)>& record) {
    require(available_, OMF_ERR_NO_DEVICE, "Vulkan context unavailable");
    ...
    require(vkAllocateCommandBuffers(device_, &alloc, &cmd) == VK_SUCCESS,
            OMF_ERR_UNKNOWN, "vkAllocateCommandBuffers failed");
    ...
    require(wait == VK_SUCCESS, OMF_ERR_UNKNOWN, "compute dispatch timed out");
}
...
VkDescriptorSet VulkanContext::allocate_set(VkDescriptorSetLayout layout,
    ...
    require(vkAllocateDescriptorSets(device_, &alloc, &set) == VK_SUCCESS,
            OMF_ERR_OUT_OF_MEMORY, "vkAllocateDescriptorSets failed");
