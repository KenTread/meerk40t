...
// omf_render_settings_t, not a copy: writing scene.camera.zoom mutates
...
struct LuaEngine::Impl {
    sol::state lua;
    omf_render_settings_t* bound{nullptr};
...
void LuaEngine::bind_types() {
    ...
    lua.new_usertype<omf_camera_transform_t>(
        "Camera",
        "distance",    &omf_camera_transform_t::distance,
        "zoom",        &omf_camera_transform_t::zoom,
        "fov",         &omf_camera_transform_t::fov_degrees,
        "near_clip",   &omf_camera_transform_t::near_clip,
        "far_clip",    &omf_camera_transform_t::far_clip,
        "dof_focal_plane", &omf_camera_transform_t::dof_focal_plane,
        "dof_strength",    &omf_camera_transform_t::dof_strength,
        "stereo_ipd",  &omf_camera_transform_t::stereo_ipd,
        "stereo_mode", &omf_camera_transform_t::stereo_mode,
        "projection",  &omf_camera_transform_t::projection,
        // Vectors are exposed as get/set pairs rather than tables so a
        // script cannot leave a half-assigned position behind.
        "get_position", [](omf_camera_transform_t& c) {
            return std::make_tuple(c.position[0], c.position[1], c.position[2]);
        },
        "set_position", [](omf_camera_transform_t& c, double x, double y, double z) {
            c.position[0] = x; c.position[1] = y; c.position[2] = z;
        },
        "get_target", [](omf_camera_transform_t& c) {
            return std::make_tuple(c.target[0], c.target[1], c.target[2]);
        },
        "set_target", [](omf_camera_transform_t& c, double x, double y, double z) {
            c.target[0] = x; c.target[1] = y; c.target[2] = z;
        },
        "get_rotation", [](omf_camera_transform_t& c) {
            return std::make_tuple(c.rotation[0], c.rotation[1], c.rotation[2]);
        },
        "set_rotation", [](omf_camera_transform_t& c, double x, double y, double z) {
            c.rotation[0] = x; c.rotation[1] = y; c.rotation[2] = z;
        });

    lua.new_usertype<omf_formula_slot_t>(
        "FormulaSlot",
        "enabled",          &omf_formula_slot_t::enabled,
        "kind",             &omf_formula_slot_t::kind,
        "hybrid_mode",      &omf_formula_slot_t::hybrid_mode,
        "iteration_weight", &omf_formula_slot_t::iteration_weight,
        "start_iteration",  &omf_formula_slot_t::start_iteration,
        "stop_iteration",   &omf_formula_slot_t::stop_iteration,
        "julia_enabled",    &omf_formula_slot_t::julia_enabled,
        "name", sol::property(
            [](omf_formula_slot_t& s) {
                return std::string(s.name, ::strnlen(s.name, OMF_MAX_NAME));
            },
            [](omf_formula_slot_t& s, const std::string& v) {
                const std::size_t n = std::min(v.size(), sizeof(s.name) - 1);
                std::memcpy(s.name, v.data(), n);
                s.name[n] = '\0';
            }),
        "get_param", [](omf_formula_slot_t& s, int i) -> double {
            return (i >= 1 && i <= OMF_FORMULA_PARAMS) ? s.params[i - 1] : 0.0;
        },
        "set_param", [](omf_formula_slot_t& s, int i, double v) {
            if (i >= 1 && i <= OMF_FORMULA_PARAMS) s.params[i - 1] = static_cast<float>(v);
        });

    lua.new_usertype<omf_calculation_settings_t>(
        "Calculation",
        "max_iterations",      &omf_calculation_settings_t::max_iterations,
        "escape_radius",       &omf_calculation_settings_t::escape_radius,
        "raystep_multiplier",  &omf_calculation_settings_t::raystep_multiplier,
        "stepwidth_limiter",   &omf_calculation_settings_t::stepwidth_limiter,
        "de_stop",             &omf_calculation_settings_t::de_stop_criterion,
        "de_max_steps",        &omf_calculation_settings_t::de_max_steps,
        "binary_search_steps", &omf_calculation_settings_t::binary_search_steps,
        "de_scale",            &omf_calculation_settings_t::de_scale);

    lua.new_usertype<omf_coloring_settings_t>(
        "Coloring",
        "trap_kind",     &omf_coloring_settings_t::trap_kind,
        "trap_size",     &omf_coloring_settings_t::trap_size,
        "trap_influence",&omf_coloring_settings_t::trap_influence,
        "color_speed",   &omf_coloring_settings_t::color_speed,
        "color_offset",  &omf_coloring_settings_t::color_offset,
        "color_cycle",   &omf_coloring_settings_t::color_cycle,
        "get_gradient", [](omf_coloring_settings_t& c, int i) -> std::uint32_t {
            return (i >= 1 && i <= OMF_GRADIENT_ENTRIES) ? c.gradient[i - 1] : 0u;
        },
        "set_gradient", [](omf_coloring_settings_t& c, int i, std::uint32_t v) {
            if (i >= 1 && i <= OMF_GRADIENT_ENTRIES) c.gradient[i - 1] = v;
        });

    lua.new_usertype<omf_light_channel_t>(
        "LightChannel",
        "kind",      &omf_light_channel_t::kind,
        "enabled",   &omf_light_channel_t::enabled,
        "intensity", &omf_light_channel_t::intensity,
        "falloff",   &omf_light_channel_t::falloff,
        "specular",  &omf_light_channel_t::specular,
        "diffuse",   &omf_light_channel_t::diffuse,
        "set_color", [](omf_light_channel_t& l, double r, double g, double b) {
            l.color[0] = static_cast<float>(r);
            l.color[1] = static_cast<float>(g);
            l.color[2] = static_cast<float>(b);
        },
        "set_position", [](omf_light_channel_t& l, double x, double y, double z) {
    ...
    sol::table log = lua.create_named_table("omf");
    log.set_function("version", [] {
        return std::to_string(OMF_VERSION_MAJOR) + "." + std::to_string(OMF_VERSION_MINOR);
    });
}

void LuaEngine::bind_settings(omf_render_settings_t* settings) {
    require(settings != nullptr, OMF_ERR_INVALID_ARG, "cannot bind a null settings pointer");
    ...
    for (int i = 0; i < OMF_FORMULA_SLOTS; ++i) {
...
void LuaEngine::run_string(std::string_view source) {
    ...
    if (!result.valid()) {
        ...
        fail(OMF_ERR_LUA, std::string("Lua error: ") + err.what());
    }
}
...
LuaEngine::LuaEngine(FormulaRegistry& registry) : registry_(registry), impl_(new Impl) {
    fail(OMF_ERR_UNSUPPORTED,
         "scripting requires LuaJIT; rebuild with -DOMF_WITH_LUA=ON");
}

LuaEngine::~LuaEngine() = default;
void LuaEngine::bind_types() {}
void LuaEngine::bind_settings(omf_render_settings_t*) {}
