...
void set_id(omf_precision_profile_t& profile, const char* id) {
...
omf_precision_profile_t make_modern() {
    omf_precision_profile_t p{};
    set_id(p, OMF_PROFILE_MODERN);
    p.tier = OMF_PRECISION_FP64;
    p.de_method = OMF_DE_AUTO;
    p.downgrade_chain_on_mixed_de = 0;
    p.probe_epsilon = 1.0e-6f;
    p.bailout_compare = OMF_BAILOUT_R2_GT;
...
omf_precision_profile_t make_legacy_19912() {
    omf_precision_profile_t p{};
    set_id(p, OMF_PROFILE_LEGACY_19912);
    p.tier = OMF_PRECISION_FP64;
...
omf_precision_profile_t make_legacy_19935() {
    omf_precision_profile_t p = make_legacy_19912();
    set_id(p, OMF_PROFILE_LEGACY_19935);
...
const std::vector<omf_precision_profile_t>& registry() {
    static const std::vector<omf_precision_profile_t> profiles = {
...
double pixel_angular_width(const omf_render_settings_t& settings, int width) {
...
bool chain_is_analytic(const omf_render_settings_t& settings) {
...
const omf_precision_profile_t* find_profile(std::string_view id) {
    for (const omf_precision_profile_t& profile : registry()) {
...
void evaluate_keyframes(const std::vector<omf_keyframe_t>& keyframes, int frame,
    ...
    const omf_keyframe_t& k1 = keyframes[index];
    const omf_keyframe_t& k2 = keyframes[index + 1];
    const omf_keyframe_t& k0 = keyframes[index > 0 ? index - 1 : index];
    const omf_keyframe_t& k3 = keyframes[index + 2 <= last ? index + 2 : last];
    ...
    omf_camera_transform_t camera = k1.camera;
    for (int i = 0; i < 3; ++i) {
