...
void write_exr(const FrameBuffer& fb, const std::string& path, bool multilayer) {
    const Layer* beauty = fb.layer(OMF_LAYER_RGBA32F);
    require(beauty != nullptr, OMF_ERR_INVALID_ARG,
            "EXR export needs the RGBA32F layer -- add OMF_LAYER_RGBA32F to layer_mask");
    ...
    if (multilayer) {
        if (const Layer* depth = fb.layer(OMF_LAYER_DEPTH)) {
        ...
        if (const Layer* normal = fb.layer(OMF_LAYER_NORMAL)) {
        ...
        if (const Layer* ao = fb.layer(OMF_LAYER_SSAO)) {
            planes.push_back({"AO.V", extract_plane(*ao, 0, 1)});
        }
        if (const Layer* shadow = fb.layer(OMF_LAYER_SHADOW)) {
            planes.push_back({"Shadow.V", extract_plane(*shadow, 0, 1)});
        }
        if (const Layer* motion = fb.layer(OMF_LAYER_MOTION)) {
    ...
    for (int i = 0; i < channel_count; ++i) {
        ...
        require(name.size() < sizeof(channels[i].name), OMF_ERR_INVALID_ARG,
    ...
    if (rc != TINYEXR_SUCCESS) {
        ...
        fail(OMF_ERR_IO, "EXR write failed: " + message);
    }
}

#else  // !OMF_WITH_TINYEXR

void write_exr(const FrameBuffer&, const std::string&, bool) {
    fail(OMF_ERR_UNSUPPORTED,
         "EXR export requires tinyexr; rebuild with -DOMF_WITH_TINYEXR=ON");
}
