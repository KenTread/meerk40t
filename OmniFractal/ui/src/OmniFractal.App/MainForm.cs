...
using OmniFractal.Formats;
using OmniFractal.Interop;
using WeifenLuo.WinFormsUI.Docking;

namespace OmniFractal.App;
...
public sealed class MainForm : Form
{
    ...
    public MainForm(NativeCore core)
    {
        ...
        Text = $"OmniFractal — {_core.Backend} ({_core.DeviceName})";
        Width = 1600;
        Height = 950;
        AllowDrop = true;
    ...
    private void LoadFile(string path)
    {
        try
        {
            OMFFileKind kind = OMFFileParser.Identify(path);

            switch (kind)
            {
                case OMFFileKind.Parameters:
                case OMFFileKind.Image:
                case OMFFileKind.Animation:
                case OMFFileKind.Lighting:
                case OMFFileKind.Gradient:
                {
                    using Scene scene = NativeCore.LoadScene(path);
                    _settings = scene.GetSettings();

                    string detail = kind == OMFFileKind.Image
        ...
        catch (Exception ex) when (ex is OmniFractalException or IOException or InvalidDataException)
        {
