...
//   * every failure arrives as an mb3d::Error carrying a result code,
...
//
// Build:  cmake -DMB3D_BUILD_FUZZERS=ON -DCMAKE_CXX_COMPILER=clang++
...
void check_settings_invariants(const mb3d_render_settings_t& settings) {
    ...
    for (const mb3d_formula_slot_t& slot : settings.formulas) {
        ...
        if (slot.repeat_from_slot < -1 ||
            slot.repeat_from_slot >= MB3D_FORMULA_SLOTS) {
            std::abort();
        }
    }
}

// Every harness funnels through this. An mb3d::Error is a correct
// rejection; anything else escaping is a bug in the parser.
template <typename Fn>
void run_guarded(Fn&& fn) {
    try {
        fn();
    } catch (const mb3d::Error&) {
...
//
// The same translation unit builds two different fuzzers; MB3D_FUZZ_M4D
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    run_guarded([&] {
        switch (selector) {
            case 0: {
                mb3d::legacy::ImportResult result =
                    mb3d::legacy::import_m3p(body, MB3D_PROFILE_LEGACY_19912);
                check_settings_invariants(result.settings);
                break;
            }
            case 1: {
                mb3d::legacy::ImportResult result =
                    mb3d::legacy::import_m3i(body, MB3D_PROFILE_MODERN);
                check_settings_invariants(result.settings);
                break;
            }
            case 2: {
                mb3d::legacy::ImportResult result =
                    mb3d::legacy::import_m3a(body, MB3D_PROFILE_MODERN);
                check_settings_invariants(result.settings);
            ...
            case 3: {
                mb3d::legacy::ImportResult result =
                    mb3d::legacy::import_m3l(body, MB3D_PROFILE_MODERN);
                check_settings_invariants(result.settings);
                break;
            }
            case 4: {
                ...
                (void)mb3d::legacy::extract_embedded_parameters(body);
                break;
            }
            case 5: {
                ...
                // an enabled formula.
                const mb3d::legacy::FormulaMetadata meta =
                    mb3d::legacy::parse_formula_metadata(body, MB3D_FILE_M3F);
                if (meta.has_executable_body && meta.executable_body_size == 0) {
                    std::abort();
                }
                (void)mb3d::legacy::resolve_formula_name(meta.name);
                break;
            }
            default:
                break;
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
    ...
    char path[] = "/tmp/mb3d_fuzz_XXXXXX";
    ...
    run_guarded([&] {
        mb3d::m4d::Summary summary = mb3d::m4d::read_summary(path);
        (void)summary;
        mb3d::m4d::Document document = mb3d::m4d::read(path);
