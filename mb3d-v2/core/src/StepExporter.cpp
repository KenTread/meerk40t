...
//
// When OCCT is not available the entry point returns MB3D_ERR_UNSUPPORTED
// rather than silently writing a different format.

#include "mb3d/MarchingCubes.hpp"

#if defined(MB3D_WITH_OPENCASCADE)
...
void write_step(const Mesh& mesh, const std::string& path, float scale_to_mm) {
    require(!mesh.indices.empty(), MB3D_ERR_GEOMETRY, "cannot export an empty mesh to STEP");
    ...
    require(skipped < mesh.triangle_count(), MB3D_ERR_GEOMETRY,
            "every facet was rejected during STEP face construction");

    sewing.Perform();
    TopoDS_Shape sewn = sewing.SewedShape();
    require(!sewn.IsNull(), MB3D_ERR_GEOMETRY, "OCCT sewing produced a null shape");
    ...
    require(transferred == IFSelect_RetDone, MB3D_ERR_GEOMETRY,
            "STEPControl_Writer::Transfer failed");

    const IFSelect_ReturnStatus written = writer.Write(path.c_str());
    require(written == IFSelect_RetDone, MB3D_ERR_IO,
...
#else  // !MB3D_WITH_OPENCASCADE

namespace mb3d {

void write_step(const Mesh&, const std::string&, float) {
    fail(MB3D_ERR_UNSUPPORTED,
         "STEP export requires OpenCASCADE; rebuild with -DMB3D_WITH_OPENCASCADE=ON");
}
