...
mb3d_pixel_format_t format_for_layer(mb3d_layer_t layer) {
    switch (layer) {
        case MB3D_LAYER_RGBA:    return MB3D_PF_BGRA8;
        case MB3D_LAYER_RGBA32F: return MB3D_PF_RGBA32F;
        case MB3D_LAYER_DEPTH:   return MB3D_PF_R32F;
        case MB3D_LAYER_NORMAL:  return MB3D_PF_RGB32F;
        case MB3D_LAYER_SSAO:    return MB3D_PF_R32F;
        case MB3D_LAYER_SHADOW:  return MB3D_PF_R32F;
        case MB3D_LAYER_MOTION:  return MB3D_PF_RG32F;
        case MB3D_LAYER_OBJECT:  return MB3D_PF_U32;
    }
    return MB3D_PF_BGRA8;
}
...
void write(const std::string& path, const Document& document) {
    ...
    require(file.good(), MB3D_ERR_IO, "cannot open '" + path + "' for writing");
    ...
    if (!document.keyframes.empty()) {
        require(document.keyframes.size() <= kMaxKeyframes, MB3D_ERR_LIMIT_EXCEEDED,
                "too many keyframes to serialise");
        PendingChunk chunk;
        chunk.type = CHUNK_KEYFRAMES;
        chunk.bytes.resize(document.keyframes.size() * sizeof(mb3d_keyframe_t));
    ...
    if (document.image) {
        ...
        constexpr mb3d_layer_t kOrder[] = {
            MB3D_LAYER_RGBA, MB3D_LAYER_RGBA32F, MB3D_LAYER_DEPTH, MB3D_LAYER_NORMAL,
            MB3D_LAYER_SSAO, MB3D_LAYER_SHADOW, MB3D_LAYER_MOTION, MB3D_LAYER_OBJECT};

        std::uint32_t layer_index = 0;
        for (mb3d_layer_t which : kOrder) {
    ...
    require(chunks.size() <= kMaxChunkCount, MB3D_ERR_LIMIT_EXCEEDED,
            "chunk count exceeds the container limit");

    M4dHeader header{};
    std::memcpy(header.magic, "MB4D", 4);
    header.format_version = MB3D_M4D_FORMAT_VERSION;
    header.writer_abi = MB3D_ABI_VERSION;
    ...
    for (const PendingChunk& chunk : chunks) {
        ...
        require(file.good(), MB3D_ERR_IO, "failed writing chunk payload");
    ...
    write_pod(file, header);
    file.flush();
    require(file.good(), MB3D_ERR_IO, "failed finalising '" + path + "'");
}
...
ReadContext open_and_read_header(const std::string& path) {
    ...
    require(ctx.file.good(), MB3D_ERR_IO, "cannot open '" + path + "'");

    ctx.file_size = static_cast<std::uint64_t>(ctx.file.tellg());
    require(ctx.file_size >= sizeof(M4dHeader), MB3D_ERR_CORRUPT,
            "'" + path + "' is too small to be a .m4d file");

    ctx.file.seekg(0);
    ctx.file.read(reinterpret_cast<char*>(&ctx.header), sizeof(M4dHeader));
    require(ctx.file.good(), MB3D_ERR_CORRUPT, "truncated .m4d header");

    require(std::memcmp(ctx.header.magic, "MB4D", 4) == 0, MB3D_ERR_PARSE,
    ...
    require(ctx.header.format_version == MB3D_M4D_FORMAT_VERSION,
            MB3D_ERR_FORMAT_VERSION,
            "'" + path + "' uses .m4d format version " +
            std::to_string(ctx.header.format_version) + "; this build reads version " +
            std::to_string(MB3D_M4D_FORMAT_VERSION) + " only");

    require(ctx.header.header_size >= sizeof(M4dHeader), MB3D_ERR_CORRUPT,
            "header_size is smaller than the header itself");
    require(ctx.header.chunk_count <= kMaxChunkCount, MB3D_ERR_LIMIT_EXCEEDED,
            "chunk count exceeds the container limit");

    // A file that has been truncated after writing still has a valid
    // header; this is what catches it.
    if (ctx.header.file_size != 0) {
        require(ctx.header.file_size <= ctx.file_size, MB3D_ERR_CORRUPT,
...
std::vector<M4dChunkEntry> read_directory(ReadContext& ctx) {
    ...
    require(ctx.header.directory_offset <= ctx.file_size &&
            ...
            MB3D_ERR_CORRUPT, "chunk directory lies outside the file");
    ...
    require(ctx.file.good(), MB3D_ERR_CORRUPT, "truncated chunk directory");
    ...
    for (const M4dChunkEntry& entry : directory) {
        require(entry.size_stored <= kMaxChunkBytes, MB3D_ERR_LIMIT_EXCEEDED,
                "chunk declares an implausible size");
        require(entry.offset <= ctx.file_size &&
                entry.size_stored <= ctx.file_size - entry.offset,
                MB3D_ERR_CORRUPT, "chunk payload extends past end of file");
        require(entry.compression == 0, MB3D_ERR_UNSUPPORTED,
...
std::vector<std::byte> read_payload(ReadContext& ctx, const M4dChunkEntry& entry) {
    ...
    require(ctx.file.good(), MB3D_ERR_CORRUPT, "truncated chunk payload");

    require(fnv1a32(bytes) == entry.checksum, MB3D_ERR_CORRUPT,
...
T decode_pod(std::span<const std::byte> bytes, const char* what) {
    require(bytes.size() == sizeof(T), MB3D_ERR_CORRUPT,
...
Document read(const std::string& path) {
    ...
    mb3d_render_settings_default(&document.active);
    ...
    for (const M4dChunkEntry& entry : directory) {
        ...
        switch (entry.type) {
            case CHUNK_SETTINGS_ACTIVE:
                document.active =
                    decode_pod<mb3d_render_settings_t>(bytes, "active settings");
                saw_active = true;
                break;

            case CHUNK_SETTINGS_LEGACY:
                document.legacy =
                    decode_pod<mb3d_render_settings_t>(bytes, "legacy settings");
                break;

            case CHUNK_PROVENANCE:
                document.provenance = decode_pod<mb3d_provenance_t>(bytes, "provenance");
                break;

            case CHUNK_PROFILE:
                // Redundant with the copy inside the settings blob, but a
                // host may want the profile without decoding everything.
                (void)decode_pod<mb3d_precision_profile_t>(bytes, "profile");
                break;

            case CHUNK_GRADIENT: {
                constexpr std::size_t kExpected =
                    sizeof(std::uint32_t) * MB3D_GRADIENT_ENTRIES;
                require(bytes.size() == kExpected, MB3D_ERR_CORRUPT,
            ...
            case CHUNK_KEYFRAMES: {
                require(bytes.size() % sizeof(mb3d_keyframe_t) == 0, MB3D_ERR_CORRUPT,
                        "keyframe chunk is not a whole number of keyframes");
                const std::size_t count = bytes.size() / sizeof(mb3d_keyframe_t);
                require(count <= kMaxKeyframes, MB3D_ERR_LIMIT_EXCEEDED,
            ...
            case CHUNK_NOTES: {
                require(bytes.size() <= kMaxTextBytes, MB3D_ERR_LIMIT_EXCEEDED,
            ...
            case CHUNK_IMAGE_LAYER: {
                require(bytes.size() >= sizeof(LayerChunkHeader), MB3D_ERR_CORRUPT,
                ...
                require(staged.header.width > 0 && staged.header.height > 0 &&
                        ...
                        MB3D_ERR_CORRUPT, "image layer declares implausible dimensions");
                ...
                require(staged.header.width == image_width &&
                        ...
                        MB3D_ERR_CORRUPT, "image layers disagree on dimensions");
    ...
    require(saw_active, MB3D_ERR_CORRUPT, "'" + path + "' has no settings chunk");

    // The settings blob was written by a build with the same ABI, but a
    // hand-edited file could still carry nonsense here.
    require(document.active.abi_version == MB3D_ABI_VERSION, MB3D_ERR_ABI_MISMATCH,
            "scene declares ABI " + std::to_string(document.active.abi_version) +
            ", this build is ABI " + std::to_string(MB3D_ABI_VERSION));
    require(document.active.width > 0 && document.active.height > 0 &&
            ...
            MB3D_ERR_CORRUPT, "scene declares implausible render dimensions");
    ...
    if (!staged_layers.empty()) {
        ...
        for (const StagedLayer& staged : staged_layers) {
            Layer* layer =
                document.image->layer(static_cast<mb3d_layer_t>(staged.header.layer));
            if (layer == nullptr) continue;

            // Exact size match required. A short payload would leave the
            // tail of the layer uninitialised; a long one would be a
            // silent inconsistency.
            require(staged.pixels.size() == layer->bytes.size(), MB3D_ERR_CORRUPT,
