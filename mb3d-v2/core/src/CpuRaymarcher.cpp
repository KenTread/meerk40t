...
Camera::Camera(const mb3d_camera_transform_t& t, int width, int height, double eye_offset) {
...
int bytes_per_pixel(mb3d_pixel_format_t f) {
    switch (f) {
        case MB3D_PF_BGRA8:   return 4;
        case MB3D_PF_R32F:    return 4;
        case MB3D_PF_RG32F:   return 8;
        case MB3D_PF_RGB32F:  return 12;
        case MB3D_PF_RGBA32F: return 16;
        case MB3D_PF_U32:     return 4;
    }
    return 4;
}
...
mb3d_pixel_format_t format_for(mb3d_layer_t layer) {
    switch (layer) {
...
FrameBuffer::FrameBuffer(int width, int height, std::uint32_t layer_mask)
    ...
    require(width > 0 && height > 0, MB3D_ERR_INVALID_ARG, "frame buffer has zero extent");

    constexpr mb3d_layer_t kAll[] = {
...
Layer* FrameBuffer::layer(mb3d_layer_t which) {
    for (auto& l : layers_) if (l->which == which) return l.get();
    return nullptr;
}

const Layer* FrameBuffer::layer(mb3d_layer_t which) const {
...
void ProgressReporter::configure(mb3d_progress_fn fn, mb3d_cancel_fn cancel, void* user) {
    progress_fn_ = fn;
    cancel_fn_ = cancel;
    user_ = user;
}
...
void ProgressReporter::begin(std::int64_t total_units, int tiles_total) {
    ...
    state_.store(MB3D_JOB_RUNNING, std::memory_order_release);
...
void ProgressReporter::advance(std::int64_t units, int tiles) {
    ...
    if (progress_fn_) {
        const mb3d_progress_t p = snapshot();
        progress_fn_(&p, user_);
    }
}

void ProgressReporter::finish(mb3d_job_state_t state) {
...
mb3d_progress_t ProgressReporter::snapshot() const {
    mb3d_progress_t p{};
...
Rgb CpuRaymarcher::shade(const Hit& hit, const Ray& ray) const {
    ...
    // the scene selected.
    double t = 0.0;
    if (C.trap_kind != MB3D_TRAP_NONE && hit.state.trap < 1e29) {
    ...
    for (int i = 0; i < MB3D_LIGHT_CHANNELS; ++i) {
        const mb3d_light_channel_t& ch = L.channels[i];
        if (!ch.enabled || ch.kind == MB3D_LIGHT_OFF) continue;

        Vec3 to_light;
        double distance = 1e30;
        double attenuation = 1.0;

        if (ch.kind == MB3D_LIGHT_DIRECTIONAL) {
...
void CpuRaymarcher::render_tile(FrameBuffer& fb, const Tile& tile, int eye) const {
    ...
    const double eye_offset = settings_.camera.stereo_mode != MB3D_STEREO_OFF
    ...
    Layer* rgba32 = fb.layer(MB3D_LAYER_RGBA32F);
    Layer* depth  = fb.layer(MB3D_LAYER_DEPTH);
    Layer* normal = fb.layer(MB3D_LAYER_NORMAL);
    Layer* ssao   = fb.layer(MB3D_LAYER_SSAO);
    Layer* shadow = fb.layer(MB3D_LAYER_SHADOW);
    Layer* object = fb.layer(MB3D_LAYER_OBJECT);
                ...
                for (int sx = 0; sx < ss; ++sx) {
                    ...
                    if (hit.hit) {
                        ...
                        if (shadow) {
                            ...
                            const Vec3 to_light = ch.kind == MB3D_LIGHT_DIRECTIONAL
...
void CpuRaymarcher::render(FrameBuffer& fb, ProgressReporter& progress) const {
    ...
    progress.finish(aborted.load() ? MB3D_JOB_CANCELLED : MB3D_JOB_DONE);
}
