...
mb3d_mesh_stats_t Mesh::stats(float scale_to_mm) const {
    mb3d_mesh_stats_t s{};
...
IsoSurfaceExtractor::IsoSurfaceExtractor(VulkanContext& vk, const FormulaRegistry& registry,
    ...
    require(extent.x > 0 && extent.y > 0 && extent.z > 0, MB3D_ERR_INVALID_ARG,
...
Mesh IsoSurfaceExtractor::extract_cpu(ProgressReporter& progress) {
    ...
    for (int z = 0; z + 1 < nz_; ++z) {
        if (progress.cancelled()) {
            progress.finish(MB3D_JOB_CANCELLED);
            return mesh;
        }
...
Mesh IsoSurfaceExtractor::extract(ProgressReporter& progress) {
    ...
    require(!mesh.indices.empty(), MB3D_ERR_GEOMETRY,
...
void write_stl_binary(const Mesh& mesh, const std::string& path, float scale_to_mm) {
    ...
    char header[80] = {};
    std::snprintf(header, sizeof(header), "MB3D v%d.%d binary STL -- %zu triangles",
                  MB3D_VERSION_MAJOR, MB3D_VERSION_MINOR, mesh.triangle_count());
    ...
    require(file.good(), MB3D_ERR_IO, "write failed for '" + path + "'");
}
