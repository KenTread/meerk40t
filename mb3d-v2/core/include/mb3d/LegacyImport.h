...
// and there is no code path that will: mb3d_* export functions return
// MB3D_ERR_WRITE_FORBIDDEN for every legacy kind. The native container
// is .m4d.
//
// THREAT MODEL
...
public:
    ...
    void seek(std::size_t offset) {
        require(offset <= data_.size(), MB3D_ERR_CORRUPT,
    ...
    void skip(std::size_t count) {
        require(count <= remaining(), MB3D_ERR_CORRUPT, "skip past end of buffer");
        cursor_ += count;
    }
    ...
    std::vector<T> counted(std::size_t count, std::size_t cap, const char* what, Fn&& read_one) {
        require(count <= cap, MB3D_ERR_LIMIT_EXCEEDED,
...
private:
    void need(std::size_t count) const {
        require(count <= remaining(), MB3D_ERR_CORRUPT,
...
mb3d_file_kind_t identify_bytes(std::span<const std::byte> head,
...
struct FormulaMetadata {
    ...
    mb3d_formula_class_t formula_class{MB3D_CLASS_3D};
    bool analytic_de{false};
    int  parameter_count{0};
    std::array<std::string, MB3D_FORMULA_PARAMS> parameter_names{};
    std::array<float, MB3D_FORMULA_PARAMS> parameter_defaults{};
