...
// the message in thread-local storage for mb3d_last_error().
// ---------------------------------------------------------------------
...
public:
    Error(mb3d_result_t code, std::string what)
        : code_(code), what_(std::move(what)) {}
    const char* what() const noexcept override { return what_.c_str(); }
    mb3d_result_t code() const noexcept { return code_; }
...
mb3d_result_t guard(Fn&& fn) noexcept {
    try {
        set_last_error({});
        return fn();
    } catch (const Error& e) {
        ...
        return MB3D_ERR_OUT_OF_MEMORY;
    } catch (const std::exception& e) {
        set_last_error(e.what());
        return MB3D_ERR_UNKNOWN;
    } catch (...) {
...
[[noreturn]] inline void fail(mb3d_result_t code, std::string msg) {
    throw Error(code, std::move(msg));
}

inline void require(bool cond, mb3d_result_t code, std::string msg) {
...
using FormulaFn = void (*)(IterationState&, const mb3d_formula_slot_t&);

struct FormulaDef {
    std::string name;
    mb3d_formula_kind_t kind{MB3D_FORMULA_NONE};
...
struct FormulaPipeline {
    ...
    mb3d_hybrid_mode_t master{MB3D_HYBRID_SEQUENTIAL};
...
// the marching-cubes extractor and mb3d_sample_de all call it.
...
class Camera {
public:
    explicit Camera(const mb3d_camera_transform_t& t, int width, int height,
...
struct Layer {
    ...
    int width{}, height{};
    int stride{};
    mb3d_pixel_format_t format{MB3D_PF_BGRA8};
    mb3d_layer_t which{MB3D_LAYER_RGBA};
...
public:
    ...
    Layer* layer(mb3d_layer_t which);
    const Layer* layer(mb3d_layer_t which) const;
    bool has(mb3d_layer_t which) const { return layer(which) != nullptr; }
...
struct Mesh {
    ...
    mb3d_mesh_stats_t stats(float scale_to_mm) const;
};
...
public:
    ...
    void configure(mb3d_progress_fn fn, mb3d_cancel_fn cancel, void* user);
    void begin(std::int64_t total_units, int tiles_total);
    void advance(std::int64_t units, int tiles = 0);
    void finish(mb3d_job_state_t state);
    bool cancelled() const;
    mb3d_progress_t snapshot() const;
...
private:
    ...
    std::atomic<int> state_{MB3D_JOB_IDLE};
    mutable std::atomic<bool> cancel_flag_{false};
    std::chrono::steady_clock::time_point start_{};
    mb3d_progress_fn progress_fn_{nullptr};
    mb3d_cancel_fn   cancel_fn_{nullptr};
    void*            user_{nullptr};
};
