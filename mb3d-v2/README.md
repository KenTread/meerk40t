...
MB3D-V2 **reads** the Mandelbulb 3D file suite (`.m3p`, `.m3i`, `.m3a`, `.m3l`,
...
`mb3d_migrate_parameters` therefore:
...
- Lua runs sandboxed: no `io`, `os`, `package`, `require`, `load`, and
  ...
  memory).
- Every parser has a libFuzzer harness (`-DMB3D_BUILD_FUZZERS=ON`, clang) run
  under ASan + UBSan.
...
Everything third-party is optional and gated behind `MB3D_WITH_*`. The core
builds and renders with all of them off — that is the CI configuration.

| Option | Default | Notes |
|---|---|---|
| `MB3D_WITH_VULKAN` | ON | volk + VMA |
| `MB3D_WITH_LUA` | ON | LuaJIT via sol2 |
| `MB3D_WITH_TINYEXR` | ON | multi-layer EXR |
| `MB3D_WITH_MINIZ` | ON | DEFLATE; required for legacy import |
| `MB3D_WITH_OPENCASCADE` | OFF | STEP export; **dynamic link enforced** |
| `MB3D_BUILD_FUZZERS` | OFF | requires clang |
...
```bash
dotnet build ui/src/MB3D.App/MB3D.App.csproj -c Release
```

## Layout

```
core/include/mb3d/Renderer.h    the C ABI — the whole host contract
core/include/mb3d/M4DFormat.h   native container
core/include/mb3d/LegacyImport.h  hardened importers + threat model
...
ui/src/MB3D.Interop/            [LibraryImport] boundary, blittable mirrors
ui/src/MB3D.Formats/            managed import surface
ui/src/MB3D.App/                WinForms shell
```
...
MB3D-V2 at your own installation to use them.
